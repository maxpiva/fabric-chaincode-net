...
namespace Hyperledger.Fabric.Shim.Impl
{
    public class Handler
    {
        ...
        private static ILog logger = LogProvider.GetLogger(typeof(Handler));

        private AsyncDuplexStreamingCall<ChaincodeMessage, ChaincodeMessage> chatStream;
        private ChaincodeBase chaincode;

        private Dictionary<string, bool> isTransaction;
        private Dictionary<string, BlockingCollection<ChaincodeMessage>> responseChannel;
        ...
        public void SerialSend(ChaincodeMessage message)
        {
            ...
            try
            {
                chatStream.RequestStream.WriteAsync(message).GetAwaiter().GetResult();
                if (logger.IsTraceEnabled())
         ...
         */
        [MethodImpl(MethodImplOptions.Synchronized)]
        private bool MarkIsTransaction(string channelId, string uuid, bool isTransaction)
        {
		    ...
		    this.isTransaction.Add(key, isTransaction);
		    return true;
	    }
        [MethodImpl(MethodImplOptions.Synchronized)]
	...
	{
		InvokeQueryResponseMessage(channelId, txId, ChaincodeMessage.Types.Type.QueryStateClose, 
		    new QueryStateClose { Id = queryId}.ToByteString());
	...
	private ByteString InvokeChaincodeSupport(ChaincodeMessage message)
	{
            ...
            try
		{
			...
			BlockingCollection<ChaincodeMessage> responseChannel = AquireResponseChannelForTx(channelId,txId);

			// send the message
			SerialSend(message);

			// wait for response
			ChaincodeMessage response = ReceiveChannel(responseChannel);
	...
	public Response InvokeChaincode(string channelId, string txId, string chaincodeName, List<byte[]> args)
	{
		try {
		    ...
		    ChaincodeSpec invocationSpec = new ChaincodeSpec {ChaincodeId = 
		        new ChaincodeID {Name = chaincodeName}, Input = 
		            new ChaincodeInput {Args = {args.Select(Google.Protobuf.ByteString.CopyFrom)}}};
	...
	public void HandleMessage(ChaincodeMessage message)
    {
		...
		try {
			...
			if (e.InnerException != null) throw e;
	...
	{
	    return NewEventMessage(ChaincodeMessage.Types.Type.PutState, channelId, txId, new PutState {Key = key}.ToByteString());
	}
