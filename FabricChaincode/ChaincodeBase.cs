...
using Mono.Options;
using Status = Hyperledger.Fabric.Shim.Status;
    ...
    public Channel NewPeerClientConnection()
    {
        ...
        if (tlsEnabled)
        {
            ...
            if (!string.IsNullOrEmpty(hostOverrideAuthority))
            {
                ...
                options.Add(new ChannelOption("grpc.ssl_target_name_override", hostOverrideAuthority));
            }
        }

        return new Channel(host, port, cred, options);
    }
